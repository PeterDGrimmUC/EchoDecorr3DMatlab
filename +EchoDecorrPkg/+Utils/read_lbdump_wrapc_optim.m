function Dm = read_lbdump_wrapc_optim(filename)
...
% rcl
import EchoDecorrPkg.Utils.bin.read_lbdump_c_optim;
Dm.H = [];
Dm.data = [];
...
bufSize=prod(sz(2:end));
t1 = 2 + hex2dec(char(mFile.data(startIndex:endIndex))');
[Dm.data,~] = read_lbdump_c_optim(mFile.data',t1);
